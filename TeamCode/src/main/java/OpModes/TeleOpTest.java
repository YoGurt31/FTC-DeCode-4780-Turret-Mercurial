...
public class TeleOpTest extends OpMode {
    ...
    private double aimbotRotate() {
        LLResult res = VisionSystem.getLatestResult();
        if (res == null || !res.isValid()) return gamepad1.right_stick_x;
        if (res.getFiducialResults() == null || res.getFiducialResults().isEmpty()) return gamepad1.right_stick_x;
    ...
    public void init() {
        ...
        Mercurial.gamepad1().x()
                .whileTrue(new Lambda("HoldLeftGate")
        ...
        Mercurial.gamepad1().b()
                .whileTrue(new Lambda("HoldRightGate")
                        .addRequirements(outtake)
                        .setExecute(() -> {
                            spinFlywheelToFar();
                            double rps = avgFlywheelRps();
        ...
        telemetry.addLine("TeleOp: Initialized (Mercurial)");
        telemetry.update();
    }
    ...
    public void loop() {
        ...
        telemetry.addData("Drive", "-leftY = %.2f", -gamepad1.left_stick_y);
        telemetry.addData("Rotate", "rightX = %.2f", gamepad1.right_stick_x);
        LLResult res = VisionSystem.getLatestResult();
        Integer seenId = VisionSystem.getFirstFiducialId(res);
        telemetry.addData("AimBot", !aimbotActive() ? "OFF" : (seenId != null ? ("TRACKING ID " + seenId) : "ACTIVE (no tag)"));

        telemetry.addLine("");
        telemetry.addLine("=== Intake / Sorter ===");
        telemetry.addData("Left Bumper", Mercurial.gamepad1().leftBumper().state());
        telemetry.addData("Right Bumper", Mercurial.gamepad1().rightBumper().state());

        telemetry.addLine("");
        telemetry.addLine("=== Shooter / Gates ===");
        telemetry.addData("Flywheel RPS", "avg = %.1f / target = %.1f", avgFlywheelRps(), FAR_TARGET_RPS);
        telemetry.addData("Left Gate", OuttakeSystem.leftGate().getPosition() <= 0.25 ? "CLOSED" : "OPEN");
        telemetry.addData("Right Gate", OuttakeSystem.rightGate().getPosition() <= 0.75 ? "CLOSED" : "OPEN");
